...
sealed class MainForm : Form
{
    ...
    string[] formats={"RAW8_8B","RAW10_16B","RAW10_PACKED","RAW12_16B","RAW12_PACKED","RAW14_16B","RAW14_PACKED","RAW16_16B","GRAY8","Y8","MONO8","GRAY16","Y16","MONO16","RGB24","BGR24","RGBA32","BGRA32","RGB48","BGR48","RGB_R8G8B8_48B","RGB_R10G10B10_48B","RGB_R12G12B12_48B","RGB_R14G14B14_48B","RGB_R16G16B16_48B","BGR_B8G8R8_48B","BGR_B10G10R10_48B","BGR_B12G12R12_48B","BGR_B14G14R14_48B","BGR_B16G16R16_48B","NV21","NV12","I420","YV12","YUV420P","P010"};
    string[] imageExports={"PNG","BMP","JPEG","TIFF"};
    string[] rgb8Exports={"RGB24","BGR24","RGBA32","BGRA32"};
    string[] rgb16Exports={"RGB48","BGR48","RGB_R8G8B8_48B","RGB_R10G10B10_48B","RGB_R12G12B12_48B","RGB_R14G14B14_48B","RGB_R16G16B16_48B","BGR_B8G8R8_48B","BGR_B10G10R10_48B","BGR_B12G12R12_48B","BGR_B14G14R14_48B","BGR_B16G16R16_48B"};
    ...
    void BuildUi()
    {
        Color appBg=Color.White;
        Color cardBg=Color.FromArgb(255,255,255);
        Color ink=Color.FromArgb(0,0,0);
        Color sub=Color.FromArgb(102,102,102);
        Color line=Color.FromArgb(234,234,234);

        BackColor=appBg;
        ForeColor=ink;
        Font=new Font("Segoe UI",9.2F,FontStyle.Regular,GraphicsUnit.Point);

        var root=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=2,RowCount=1,BackColor=appBg,Padding=new Padding(0)};
        root.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,368));
        ...
        var header=new Label{Text="Inspect camera frames",Dock=DockStyle.Fill,TextAlign=ContentAlignment.MiddleLeft,Font=new Font("Segoe UI Semibold",22F,FontStyle.Bold),ForeColor=ink,Padding=new Padding(2,0,0,0),BackColor=appBg};
        right.Controls.Add(header,0,0);

        var viewerFrame=new RoundedPanel{Dock=DockStyle.Fill,FillColor=cardBg,BackColor=cardBg,BorderColor=line,Radius=14,Padding=new Padding(12)};
        right.Controls.Add(viewerFrame,0,1);
        imagePanel.Dock=DockStyle.Fill; imagePanel.AutoScroll=true; imagePanel.TabStop=true; imagePanel.FillColor=Color.Black; imagePanel.BackColor=Color.Black; imagePanel.BorderColor=Color.Black; imagePanel.Radius=10; imagePanel.Padding=new Padding(0); imagePanel.BorderStyle=BorderStyle.None; imagePanel.MouseEnter+=delegate{imagePanel.Focus();}; imagePanel.MouseWheel+=ImageWheel; imagePanel.Resize+=delegate{if(multiMode)LayoutGallery();}; viewerFrame.Controls.Add(imagePanel);
        pic.Location=new Point(0,0); pic.SizeMode=PictureBoxSizeMode.StretchImage; pic.BackColor=Color.Black; pic.MouseEnter+=delegate{imagePanel.Focus();}; pic.MouseWheel+=ImageWheel; gallery.MouseEnter+=delegate{imagePanel.Focus();}; gallery.MouseWheel+=ImageWheel; imagePanel.Controls.Add(pic);

        status.Dock=DockStyle.Fill; status.TextAlign=ContentAlignment.MiddleLeft; status.Padding=new Padding(2,0,0,0); status.BackColor=appBg; status.ForeColor=sub; status.Font=new Font("Consolas",8.8F); status.Text="ready · drop or open a phone camera frame"; right.Controls.Add(status,0,2);
    ...
    void Browse()
    {
        using(var d=new OpenFileDialog())
        {
            ...
            d.Multiselect=true;
            d.Filter="Phone camera files|*.raw*;*.rgb;*.bgr;*.rgba;*.bgra;*.rgb24;*.bgr24;*.rgba32;*.bgra32;*.rgb48;*.bgr48;*.rgb_*;*.bgr_*;*.gray;*.gray8;*.y8;*.mono8;*.gray16;*.y16;*.mono16;*.nv21;*.nv12;*.i420;*.yv12;*.yuv420p;*.p010;*.yuv;*.bin;*.dat|All files|*.*";
    ...
    static int BitsForFormat(string f)
    {
        ...
        int rgbBits; if(TryRgbWordBits(k,out rgbBits))return rgbBits; return IsGray16FormatName(k)?16:8;
    }
    ...
    static bool TryRgbWordBits(string f,out int bits)
    {
        ...
        Match m=Regex.Match(k,@"^(RGB)_R(8|10|12|14|16)G\2B\2_48B$|^(BGR)_B(8|10|12|14|16)G\4R\4_48B$");
        if(!m.Success)return false;
        bits=Int32.Parse(m.Groups[2].Success?m.Groups[2].Value:m.Groups[4].Value); return true;
    }
    static bool IsRgbWordFormatName(string f){int bits; return TryRgbWordBits(f,out bits);}
    static bool IsBgrWordFormatName(string f){string k=(f??"").ToUpperInvariant(); int bits; return k.StartsWith("BGR_")&&TryRgbWordBits(k,out bits);}
    ...
    static bool PayloadToPixels(long payload,string format,out long pixels)
    {
        ...
        if(IsRgb16FormatName(f)){if(payload%6!=0)return false; pixels=payload/6; return true;}
        return false;
    }
    ...
    static int DefaultStride(int w,string f){if(IsGray8FormatName(f))return w;if(IsGray16FormatName(f))return w*2;if(f=="RAW8_8B")return w;if(f=="RAW10_PACKED")return ((w+3)/4)*5;if(f=="RAW12_PACKED")return ((w+1)/2)*3;if(f=="RAW14_PACKED")return (w*14+7)/8;if(f=="RGB24"||f=="BGR24")return w*3;if(f=="RGBA32"||f=="BGRA32")return w*4;if(IsRgb16FormatName(f))return w*6;if(f=="NV21"||f=="NV12"||f=="I420"||f=="YV12"||f=="YUV420P")return w;return w*2;}
    ...
    bool IsRgb(){return p.Format=="RGB24"||p.Format=="BGR24"||p.Format=="RGBA32"||p.Format=="BGRA32"||IsRgb16FormatName(p.Format);}
    ...
    void ShowMultiStart()
    {
        ...
        gallery.Padding=new Padding(14);
    ...
    void AddImageCard(ViewerItem job,int done,int total)
    {
        ...
        var card=new RoundedPanel{FillColor=Color.White,BackColor=Color.Black,BorderColor=Color.FromArgb(234,234,234),Radius=12,Margin=new Padding(10),Padding=new Padding(10)};
    ...
    {
        var card=new RoundedPanel{FillColor=Color.White,BackColor=Color.Black,BorderColor=Color.FromArgb(255,204,204),Radius=12,Margin=new Padding(10),Padding=new Padding(14)};
        var cap=new Label{Text=Path.GetFileName(job.Path)+"\r\nFAILED: "+job.Error,Dock=DockStyle.Fill,ForeColor=Color.FromArgb(180,35,24),TextAlign=ContentAlignment.MiddleLeft,BackColor=Color.White,Font=new Font("Consolas",8.4F)};
    ...
    {
        if(bits>=16)return (byte)(word>>8);
        int max=(1<<bits)-1, v=lsb?(word&max):(word>>(16-bits));
        if(v<0)v=0; else if(v>max)v=max;
        return Clamp((v*255+max/2)/max);
    }
    static ushort ByteToRgbWord(int value,int bits,bool lsb)
    {
        int max=(1<<bits)-1, v=(Clamp(value)*max+127)/255;
        return (ushort)(lsb?v:(v<<(16-bits)));
    }
    static void RgbAtCore(byte[] source,Params q,int x,int y,out byte r,out byte g,out byte b)
    {
        if(IsRgbWordFormatName(q.Format))
        {
            ...
            byte c0=RgbWordToByte(ReadU16Core(source,o,q.Little),bits,q.Lsb);
            byte c1=RgbWordToByte(ReadU16Core(source,o+2,q.Little),bits,q.Lsb);
            byte c2=RgbWordToByte(ReadU16Core(source,o+4,q.Little),bits,q.Lsb);
            if(IsBgrWordFormatName(q.Format)){b=c0;g=c1;r=c2;}else{r=c0;g=c1;b=c2;}
            return;
        }
    ...
    List<string> AllowedExportsFor(Params q,out string hint)
    {
        ...
        else if(IsRgb16FormatName(f))
        {
            ...
            hint="RAW output is locked: RGB48/BGR48/RGB_RxGxBx_48B/BGR_BxGxRx_48B cannot restore sensor Bayer RAW.";
        }
    ...
    byte[] EncodeConvertedFrame(Bitmap bmp,string kind)
    {
        ...
        if(kind=="RGB24"||kind=="BGR24"||kind=="RGBA32"||kind=="BGRA32"||IsRgb16FormatName(kind))return EncodeRgbDump(pix,stride,w,h,kind);
    ...
    byte[] EncodeRgbDump(byte[] pix,int stride,int w,int h,string kind)
    {
        ...
        for(int y=0;y<h;y++)for(int x=0;x<w;x++)
        {
            ...
            {
                int c0=(kind=="BGR48"||IsBgrWordFormatName(kind))?b:r,c1=g,c2=(kind=="BGR48"||IsBgrWordFormatName(kind))?r:b; bool little=endianBox.SelectedIndex==0;
                ushort v0,v1,v2;
    ...
    static class SelfTest
    {
        ...
        public static int Run(string[] args)
        {
            ...
            Test("RGB/BGR word 48B alignment decode", TestRgbSub16AlignmentDecode);
        ...
        static void TestAlignmentSuffixDetection()
        {
            ...
            EqBool("rgb8 suffix lsb",DetectLsbAlignedFromName("frame_640x480.RGB_R8G8B8_48B_LSB"),true);
            ...
            EqBool("bgr12 suffix msb",DetectLsbAlignedFromName("frame_640x480.BGR_B12G12R12_48B_MSB"),false);
            ...
            using(MainForm form=new MainForm(null)){fmt=form.DetectFormatFromExtension("frame_640x480.BGR_B12G12R12_48B_MSB",out pattern);}
            if(fmt!="BGR_B12G12R12_48B")throw new Exception("BGR12 MSB suffix format parse failed: "+fmt);
        }
        ...
        static void TestStrideAndExpectedBytes()
        {
            ...
            Eq("BGR_B12G12R12_48B bits",BitsForFormat("BGR_B12G12R12_48B"),12);
            ...
            q.Format="BGR_B12G12R12_48B"; q.Stride=48; EqLong("BGR_B12G12R12_48B expected bytes",ExpectedBytes(q),199);
        }
        ...
        static Params ParamsFromSamplePath(string path)
        {
            ...
            q.W=w; q.H=h; q.Offset=0; q.Little=true; q.Lsb=DetectLsbAlignedFromName(path); q.Pattern=0; q.YuvMatrix=0; q.YuvFullRange=false;
            string ext=Path.GetExtension(path).TrimStart('.').ToUpperInvariant();
            ext=Regex.Replace(ext,@"_(?:LSB|MSB)(?:_?ALIGNED|_?ALIGN)?$","",RegexOptions.IgnoreCase);
            ...
            else if(ext=="RGB24"||ext=="BGR24"||ext=="RGBA32"||ext=="BGRA32"||IsRgb16FormatName(ext))q.Format=ext;
            else throw new Exception("unsupported sample extension: "+ext);
            if(q.Bits==0)q.Bits=BitsForFormat(q.Format);
        ...
        static void SmokeDecodeSample(string path,Params q,string label)
        {
            ...
            else if(IsRgb16FormatName(q.Format)||q.Format=="RGB24"||q.Format=="BGR24"||q.Format=="RGBA32"||q.Format=="BGRA32")
            {
        ...
        static int ByteFromBits(int v,int bits){int max=(1<<bits)-1;return (v*255+max/2)/max;}
        static void CheckRgbSub16Decode(string label,string format,int bits,int rv,int gv,int bv)
        {
            ...
            ushort w0=IsBgrWordFormatName(format)?bw:rw, w1=gw, w2=IsBgrWordFormatName(format)?rw:bw;
            RgbAtCore(new byte[]{(byte)(w0&255),(byte)(w0>>8),(byte)(w1&255),(byte)(w1>>8),(byte)(w2&255),(byte)(w2>>8)},q,0,0,out r,out g,out b);
            ...
            q.Little=false;
            RgbAtCore(new byte[]{(byte)(w0>>8),(byte)(w0&255),(byte)(w1>>8),(byte)(w1&255),(byte)(w2>>8),(byte)(w2&255)},q,0,0,out r,out g,out b);
            ...
            q.Little=true; q.Lsb=true; rw=(ushort)rv; gw=(ushort)gv; bw=(ushort)bv; w0=IsBgrWordFormatName(format)?bw:rw; w1=gw; w2=IsBgrWordFormatName(format)?rw:bw;
            RgbAtCore(new byte[]{(byte)(w0&255),(byte)(w0>>8),(byte)(w1&255),(byte)(w1>>8),(byte)(w2&255),(byte)(w2>>8)},q,0,0,out r,out g,out b);
        ...
        static void TestRgbSub16AlignmentDecode()
        {
            CheckRgbSub16Decode("RGB8","RGB_R8G8B8_48B",8,0x40,0x80,0xFF);
            CheckRgbSub16Decode("RGB10","RGB_R10G10B10_48B",10,0x100,0x200,0x3FF);
            CheckRgbSub16Decode("RGB12","RGB_R12G12B12_48B",12,0x400,0x800,0xFFF);
            CheckRgbSub16Decode("RGB14","RGB_R14G14B14_48B",14,0x1000,0x2000,0x3FFF);
            CheckRgbSub16Decode("RGB16","RGB_R16G16B16_48B",16,0x4000,0x8000,0xFFFF);
            CheckRgbSub16Decode("BGR12","BGR_B12G12R12_48B",12,0x400,0x800,0xFFF);
        }
