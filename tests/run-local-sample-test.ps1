param(
    [string]$SampleDir = "C:\Users\ljr23225\Desktop\output"
)
...
try {
    ...
    if (-not (Test-Path -LiteralPath $SampleDir)) { throw "SampleDir not found: $SampleDir" }
    if (Test-Path $report) { Remove-Item -LiteralPath $report -Force }
    $process = Start-Process -FilePath $exe -ArgumentList @("--self-test", $report, "--sample-dir", $SampleDir) -WindowStyle Hidden -Wait -PassThru
    if ($process.ExitCode -ne 0) {
        if (Test-Path $report) { Get-Content -LiteralPath $report }
        throw "Local sample test failed with exit code $($process.ExitCode)"
    }
